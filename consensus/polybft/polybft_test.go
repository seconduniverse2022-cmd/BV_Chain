...
// the test initializes bvbft and chain mock (map of headers) after which a new header is verified
...
func Testbvbft_VerifyHeader(t *testing.T) {
	t.Parallel()
	...
	for i := uint64(1); i < bvbftConfig.EpochSize; i++ {
	...
	// create bvbft with appropriate mocks
	bvbft := &bvbft{
	...
	currentHeader := &types.Header{
		Number:     bvbftConfig.EpochSize + 1,
		ParentHash: parentHeader.Hash,
		Timestamp:  parentHeader.Timestamp + 1,
		MixHash:    bvbftMixDigest,
		Difficulty: 1,
	}
	...
	assert.ErrorContains(t, bvbft.VerifyHeader(currentHeader), "invalid header hash")
	...
	assert.ErrorContains(t, bvbft.VerifyHeader(currentHeader), "failed to verify signatures for parent of block")
	...
	assert.NoError(t, bvbft.VerifyHeader(currentHeader))
	...
	bvbft.validatorsCache = newValidatorsSnapshotCache(hclog.NewNullLogger(), newTestState(t), blockchainMock)
	assert.NoError(t, bvbft.validatorsCache.storeSnapshot(
...
func Testbvbft_Close(t *testing.T) {
	t.Parallel()
	...
	bvbft.closeCh = make(chan struct{})

	assert.Error(t, errExpected, bvbft.Close())
...
func Testbvbft_GetSyncProgression(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, result, bvbft.GetSyncProgression())
}
...
func Test_Factory(t *testing.T) {
	...
	assert.Equal(t, txPool, bvbft.txPool)
	assert.Equal(t, epochSize, bvbft.consensusConfig.EpochSize)
	assert.Equal(t, params, bvbft.config)
}
