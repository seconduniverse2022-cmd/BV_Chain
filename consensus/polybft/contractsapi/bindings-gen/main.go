...
import (
	...
	gensc "github.com/bitvault/consensus/bvbft/contractsapi"
...
`
	...
	if err = common.SaveFileSafe("./consensus/bvbft/contractsapi/contractsapi.go", output, 0600); err != nil {
		log.Fatal(err)
	}
}
...
func generateType(generatedData *generatedData, name string, obj *abi.Type, res *[]string) (string, error) {
	...
	for _, tupleElem := range obj.TupleElems() {
		...
		// uint8 internally in bvbft, we can use regexp to replace those values with the
