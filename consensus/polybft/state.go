...
var (
	bitvaultEventsLastProcessedBlockBucket = []byte("bitvaultEventsLastProcessedBlock")
	bitvaultEventsLastProcessedBlockKey    = []byte("bitvaultEventsLastProcessedBlockKey")
)
...
func (s *State) initStorages() error {
	...
	return s.db.Update(func(tx *bolt.Tx) error {
		...
		_, err := tx.CreateBucketIfNotExists(bitvaultEventsLastProcessedBlockBucket)
		if err != nil {
			return fmt.Errorf("failed to create bucket=%s: %w", string(bitvaultEventsLastProcessedBlockBucket), err)
		}
		...
		if lastProcessedBlock == 0 {
			// we do this for already existing chains, which just updated their bitvault binary,
...
// insertLastProcessedEventsBlock inserts the last processed block for events on bitvault
func (s *State) insertLastProcessedEventsBlock(block uint64, dbTx *bolt.Tx) error {
	insertFn := func(tx *bolt.Tx) error {
		return tx.Bucket(bitvaultEventsLastProcessedBlockBucket).Put(
			bitvaultEventsLastProcessedBlockKey, common.EncodeUint64ToBytes(block))
	}
...
// getLastProcessedEventsBlock gets the last processed block for events on bitvault
func (s *State) getLastProcessedEventsBlock(dbTx *bolt.Tx) (uint64, error) {
	...
	getFn := func(tx *bolt.Tx) {
		value := tx.Bucket(bitvaultEventsLastProcessedBlockBucket).Get(bitvaultEventsLastProcessedBlockKey)
		if value != nil {
