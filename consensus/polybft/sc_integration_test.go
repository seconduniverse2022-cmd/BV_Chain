...
func TestIntegration_CommitEpoch(t *testing.T) {
	...
	for _, currentValidators := range validatorSets {
		...
		// validator data for bvbft config
		...
		for i, val := range accSet {
			...
			// create validator data for bvbft config
		...
		// init ValidatorSet
		err := initValidatorSet(bvbftConfig, transition)
		require.NoError(t, err)

		// init RewardPool
		err = initRewardPool(bvbftConfig, transition)
		require.NoError(t, err)

		// approve reward pool as reward token spender
		err = approveRewardPoolAsSpender(bvbftConfig, transition)
		require.NoError(t, err)

		// create input for commit epoch
		commitEpoch := createTestCommitEpochInput(t, 1, bvbftConfig.EpochSize)
		...
		distributeRewards := createTestDistributeRewardsInput(t, 1, accSet, bvbftConfig.EpochSize)
		...
		commitEpoch = createTestCommitEpochInput(t, 2, bvbftConfig.EpochSize)
		input, err = commitEpoch.EncodeAbi()
		require.NoError(t, err)
		...
		distributeRewards = createTestDistributeRewardsInput(t, 2, accSet, bvbftConfig.EpochSize)
		input, err = distributeRewards.EncodeAbi()
		require.NoError(t, err)
