...
func TestFSM_BuildProposal_WithoutCommitEpochTxGood(t *testing.T) {
	...
	fsm := &fsm{parent: parent, blockBuilder: mBlockBuilder, config: &bvbftConfig{}, backend: blockchainMock,
...
func TestFSM_BuildProposal_NonEpochEndingBlock_ValidatorsDeltaNil(t *testing.T) {
	...
	fsm := &fsm{parent: parent, blockBuilder: blockBuilderMock,
		config: &bvbftConfig{}, backend: &blockchainMock{},
...
func TestFSM_Validate_ExitEventRootNotExpected(t *testing.T) {
	...
	bvbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.GetPublicIdentities(), nil).Once()
	...
	fsm := &fsm{
		...
		config:            &bvbftConfig{BlockTimeDrift: 1},
...
func TestFSM_Validate_FailToVerifySignatures(t *testing.T) {
	...
	bvbftBackendMock := new(bvbftBackendMock)
	bvbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validatorsMetadata, nil).Once()
