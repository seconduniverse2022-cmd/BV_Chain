...
func TestExtra_ValidateFinalizedData_UnhappyPath(t *testing.T) {
	...
	bvBackendMock := new(bvbftBackendMock)
	bvBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(nil, errors.New("validators not found"))
	...
	err = extra.ValidateFinalizedData(
		header, parent, nil, chainID, bvBackendMock, signer.DomainCheckpointManager, hclog.NewNullLogger())
	...
	bvBackendMock = new(bvbftBackendMock)
	bvBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.GetPublicIdentities())
...
func TestExtra_ValidateParentSignatures(t *testing.T) {
	...
	bvBackendMock := new(bvbftBackendMock)
	bvBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(nil, errors.New("no validators"))

	// validation is skipped for blocks 0 and 1
	extra := &Extra{}
	err := extra.ValidateParentSignatures(
		1, bvBackendMock, nil, nil, nil, chainID, signer.DomainCheckpointManager, hclog.NewNullLogger())
	require.NoError(t, err)

	// parent signatures not present
	err = extra.ValidateParentSignatures(
		headerNum, bvBackendMock, nil, nil, nil, chainID, signer.DomainCheckpointManager, hclog.NewNullLogger())
	...
	err = extra.ValidateParentSignatures(
		headerNum, bvBackendMock, nil, parent, parentExtra, chainID, signer.DomainCheckpointManager, hclog.NewNullLogger())
	require.ErrorContains(t, err,
