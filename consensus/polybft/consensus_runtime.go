...
import (
	...
	"github.com/bitvault/go-ibft/messages"
	"github.com/bitvault/go-ibft/messages/proto"
...
type runtimeConfig struct {
	bvbftConfig           *bvbftConfig
	DataDir               string
	Key                   *wallet.Key
	State                 *State
	blockchain            blockchainBackend
	bvbftBackend          bvbftBackend
...
func (c *consensusRuntime) initStateSyncManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		stateSenderAddr := c.config.bvbftConfig.Bridge.StateSenderAddr
		stateSyncManager := newStateSyncManager(
			...
			&stateSyncConfig{
				...
				stateSenderStartBlock:    c.config.bvbftConfig.Bridge.EventTrackerStartBlocks[stateSenderAddr],
				jsonrpcAddr:              c.config.bvbftConfig.Bridge.JSONRPCEndpoint,
				dataDir:                  c.config.DataDir,
				topic:                    c.config.bridgeTopic,
				maxCommitmentSize:        maxCommitmentSize,
				numBlockConfirmations:    c.config.numBlockConfirmations,
				blockTrackerPollInterval: c.config.bvbftConfig.BlockTrackerPollInterval.Duration,
			},
			c,
		)
...
func (c *consensusRuntime) initCheckpointManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		...
		txRelayer, err := txrelayer.NewTxRelayer(
			txrelayer.WithIPAddress(c.config.bvbftConfig.Bridge.JSONRPCEndpoint),
		...
		c.checkpointManager = newCheckpointManager(
			...
			c.config.bvbftConfig.Bridge.CheckpointManagerAddr,
			txRelayer,
			c.config.blockchain,
			c.config.bvbftBackend,
...
func (c *consensusRuntime) initStakeManager(logger hcf.Logger, dbTx *bolt.Tx) error {
	rootRelayer, err := txrelayer.NewTxRelayer(txrelayer.WithIPAddress(c.config.bvbftConfig.Bridge.JSONRPCEndpoint))
	if err != nil {
		return err
	}
	...
	c.stakeManager, err = newStakeManager(
		...
		c.config.bvbftConfig.Bridge.CustomSupernetManagerAddr,
		c.config.blockchain,
		c.config.bvbftBackend,
		int(c.config.bvbftConfig.MaxValidatorSetSize),
		dbTx,
	)
...
func (c *consensusRuntime) IsBridgeEnabled() bool {
	return c.config.bvbftConfig.IsBridgeEnabled()
}
...
func (c *consensusRuntime) FSM() error {
	...
	blockBuilder, err := c.config.blockchain.NewBlockBuilder(
		...
		c.config.txPool,
		c.config.bvbftConfig.BlockTime.Duration,
		c.logger,
	)
	...
	ff := &fsm{
		config:            c.config.bvbftConfig,
		parent:            parent,
		backend:           c.config.blockchain,
		bvbftBackend:      c.config.bvbftBackend,
...
func (c *consensusRuntime) restartEpoch(header *types.Header, dbTx *bolt.Tx) (*epochMetadata, error) {
	...
	validatorSet, err := c.config.bvbftBackend.GetValidatorsWithTx(header.Number, nil, dbTx)
	if err != nil {
	...
	*contractsapi.DistributeRewardForRewardPoolFn, error) {
	...
	if blockHeader.Number > commitEpochLookbackSize {
		for i := 0; i < commitEpochLookbackSize; i++ {
			validators, err := c.config.bvbftBackend.GetValidators(blockHeader.Number-2, nil)
			if err != nil {
				return nil, nil, err
			}
...
func (c *consensusRuntime) isFixedSizeOfEpochMet(blockNumber uint64, epoch *epochMetadata) bool {
	return epoch.FirstBlockInEpoch+c.config.bvbftConfig.EpochSize-1 == blockNumber
}
...
func (c *consensusRuntime) isFixedSizeOfSprintMet(blockNumber uint64, epoch *epochMetadata) bool {
	return (blockNumber-epoch.FirstBlockInEpoch+1)%c.config.bvbftConfig.SprintSize == 0
}
