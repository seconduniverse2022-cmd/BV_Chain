...
const (
	...
	defaultNativeTokenName     = "bitvault"
...
// bvbftConfig is the configuration file for the bvbft consensus protocol.
type bvbftConfig struct {
...
// LoadbvbftConfig loads chain config from provided path and unmarshals bvbftConfig
func LoadbvbftConfig(chainConfigFile string) (bvbftConfig, error) {
	chainCfg, err := chain.ImportFromFile(chainConfigFile)
	if err != nil {
		return bvbftConfig{}, err
	}

	bvbftConfig, err := GetbvbftConfig(chainCfg)
	if err != nil {
		return bvbftConfig{}, err
	}
...
// GetbvbftConfig deserializes provided chain config and returns bvbftConfig
func GetbvbftConfig(chainConfig *chain.Chain) (bvbftConfig, error) {
	consensusConfigJSON, err := json.Marshal(chainConfig.Params.Engine[ConsensusName])
	if err != nil {
		return bvbftConfig{}, err
	}

	var bvbftConfig bvbftConfig
	if err = json.Unmarshal(consensusConfigJSON, &bvbftConfig); err != nil {
		return bvbftConfig{}, err
	}

	return bvbftConfig, nil
}
...
func (p *bvbftConfig) IsBridgeEnabled() bool {
	return p.Bridge != nil
}
...
// TokenConfig is the configuration of native token used by bitvault network
