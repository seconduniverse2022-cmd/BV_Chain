...
type fsm struct {
	// bvbft consensus protocol configuration
	config *bvbftConfig
	...
	// bvbftBackend implements methods needed from the bvbft
...
func (f *fsm) Validate(proposal []byte) error {
	...
	if err := extra.ValidateParentSignatures(block.Number(), f.bvbftBackend, nil, f.parent, parentExtra,
	...
	if f.logger.IsTrace() && block.Number() > 1 {
		validators, err := f.bvbftBackend.GetValidators(block.Number()-2, nil)
		if err != nil {
...
func validateHeaderFields(parent *types.Header, header *types.Header, blockTimeDrift uint64) error {
	...
	// verify mix digest
	if header.MixHash != bvbftMixDigest {
