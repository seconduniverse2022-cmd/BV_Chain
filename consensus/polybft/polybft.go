// Package bvbft implements PBFT consensus algorithm integration and bridge feature
package bvbft
...
import (
	...
	"github.com/bitvault/helper/progress"
	"github.com/bitvault/network"
	"github.com/bitvault/state"
	"github.com/bitvault/syncer"
...
// bvbftBackend is an interface defining bvbft methods needed by fsm and sync tracker
type bvbftBackend interface {
...
func Factory(params *consensus.Params) (consensus.Consensus, error) {
	logger := params.Logger.Named("bvbft")
	...
	// initialize bvbft consensus config
	customConfigJSON, err := json.Marshal(params.Config.Config)
	if err != nil {
		return nil, err
	}

	err = json.Unmarshal(customConfigJSON, &bvbft.consensusConfig)
	if err != nil {
		return nil, err
	}
...
type bvbft struct {
	...
	// consensusConfig is genesis configuration for bvbft consensus protocol
	consensusConfig *bvbftConfig
...
func GenesisPostHookFactory(config *chain.Chain, engineName string) func(txn *state.Transition) error {
	return func(transition *state.Transition) error {
		bvbftConfig, err := GetbvbftConfig(config)
		if err != nil {
			return err
		}
		...
		burnContractAddress, isBurnContractSet := getBurnContractAddress(config, bvbftConfig)
		...
		if err = initProxies(transition, bvbftConfig.ProxyContractsAdmin, proxyAddrMapping); err != nil {
			return err
		}

		// initialize ValidatorSet SC
		if err = initValidatorSet(bvbftConfig, transition); err != nil {
			return err
		}

		// approve reward pool
		if err = approveRewardPoolAsSpender(bvbftConfig, transition); err != nil {
			return err
		}

		// mint reward tokens to reward wallet
		if err = mintRewardTokensToWallet(bvbftConfig, transition); err != nil {
			return err
		}

		// initialize RewardPool SC
		if err = initRewardPool(bvbftConfig, transition); err != nil {
			return err
		}
		...
		if bridgeAllowListAdmin != types.ZeroAddress || bridgeBlockListAdmin != types.ZeroAddress {
			...
			input, err := getInitERC20PredicateACLInput(bvbftConfig.Bridge, owner,
			...
			input, err = getInitERC721PredicateACLInput(bvbftConfig.Bridge, owner,
				useBridgeAllowList, useBridgeBlockList, false)
			if err != nil {
				return err
			}
			...
			input, err = getInitERC1155PredicateACLInput(bvbftConfig.Bridge, owner,
				useBridgeAllowList, useBridgeBlockList, false)
			if err != nil {
				return err
			}
		...
		if bvbftConfig.NativeTokenConfig.IsMintable {
			...
			params := &contractsapi.InitializeNativeERC20MintableFn{
				...
				Owner_:       bvbftConfig.NativeTokenConfig.Owner,
				RootToken_:   types.ZeroAddress, // in case native mintable token is used, it is always root token
				Name_:        bvbftConfig.NativeTokenConfig.Name,
				Symbol_:      bvbftConfig.NativeTokenConfig.Symbol,
				Decimals_:    bvbftConfig.NativeTokenConfig.Decimals,
			...
			params := &contractsapi.InitializeNativeERC20Fn{
				...
				Symbol_:      bvbftConfig.NativeTokenConfig.Symbol,
				Decimals_:    bvbftConfig.NativeTokenConfig.Decimals,
				RootToken_:   bvbftConfig.Bridge.RootNativeERC20Addr,
				Predicate_:   contracts.ChildERC20PredicateContract,
				TokenSupply_: initialTotalSupply,
			}
...
func (p *bvbft) Initialize() error {
	p.logger.Info("initializing bvbft...")
	...
	// initialize bvbft consensus data directory
	p.dataDir = filepath.Join(p.config.Config.Path, "bvbft")
...
func ForkManagerInitialParamsFactory(config *chain.Chain) (*forkmanager.ForkParams, error) {
	pbftConfig, err := GetbvbftConfig(config)
	if err != nil {
		return nil, err
	}
...
func (p *bvbft) Start() error {
	p.logger.Info("starting bvbft consensus", "signer", p.key.String())
...
func (p *bvbft) initRuntime() error {
	runtimeConfig := &runtimeConfig{
		bvbftConfig:           p.consensusConfig,
		Key:                   p.key,
		DataDir:               p.dataDir,
		State:                 p.state,
...
func (p *bvbft) startRuntime() error {
	go p.startConsensusProtocol()

	return nil
}

func (p *bvbft) startConsensusProtocol() {
...
func (p *bvbft) waitForNPeers() bool {
	for {
		select {
		case <-p.closeCh:
			return false
...
func (p *bvbft) GetSyncProgression() *progress.Progression {
...
func (p *bvbft) VerifyHeader(header *types.Header) error {
...
func (p *bvbft) verifyHeaderImpl(parent, header *types.Header, blockTimeDrift uint64, parents []*types.Header) error {
...
func (p *bvbft) GetValidators(blockNumber uint64, parents []*types.Header) (validator.AccountSet, error) {
	return p.validatorsCache.GetSnapshot(blockNumber, parents, nil)
}

func (p *bvbft) GetValidatorsWithTx(blockNumber uint64, parents []*types.Header,
	dbTx *bolt.Tx) (validator.AccountSet, error) {
...
func (p *bvbft) ProcessHeaders(_ []*types.Header) error {
	// Not required
	return nil
}

// GetBlockCreator retrieves the block creator (or signer) given the block header
func (p *bvbft) GetBlockCreator(h *types.Header) (types.Address, error) {
	return types.BytesToAddress(h.Miner), nil
}

// PreCommitState a hook to be called before finalizing state transition on inserting block
func (p *bvbft) PreCommitState(block *types.Block, _ *state.Transition) error {
...
func (p *bvbft) GetBridgeProvider() consensus.BridgeDataProvider {
	return p.runtime
}

// FilterExtra is an implementation of Consensus interface
func (p *bvbft) FilterExtra(extra []byte) ([]byte, error) {
...
func getBurnContractAddress(config *chain.Chain, bvbftConfig bvbftConfig) (types.Address, bool) {
	if config.Params.BurnContract != nil &&
		len(config.Params.BurnContract) == 1 &&
		!bvbftConfig.NativeTokenConfig.IsMintable {
