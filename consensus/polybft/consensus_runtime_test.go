...
func TestConsensusRuntime_isFixedSizeOfEpochMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.bvbftConfig.EpochSize = c.epochSize
...
func TestConsensusRuntime_isFixedSizeOfSprintMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.bvbftConfig.SprintSize = c.sprintSize
		runtime.epoch.FirstBlockInEpoch = c.firstBlockInEpoch
		assert.False(t,
...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	bvbftBackendMock := new(bvbftBackendMock)
	bvbftBackendMock.On("GetValidatorsWithTx", mock.Anything, mock.Anything, mock.Anything).Return(validatorSet).Times(3)
...
func TestConsensusRuntime_OnBlockInserted_MiddleOfEpoch(t *testing.T) {
	...
	bvbftBackendMock := new(bvbftBackendMock)
	bvbftBackendMock.On("GetValidatorsWithTx", mock.Anything, mock.Anything, mock.Anything).Return(nil).Once()
	...
	config := &runtimeConfig{
		bvbftConfig: &bvbftConfig{EpochSize: epochSize},
		blockchain:  blockchainMock,
		txPool:      txPool,
	}
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	bvbftBackendMock := new(bvbftBackendMock)
	bvbftBackendMock.On("GetValidatorsWithTx", mock.Anything, mock.Anything, mock.Anything).Return(validators).Times(3)
	...
	assert.Equal(t, uint64(10), runtime.config.bvbftConfig.SprintSize)
	assert.Equal(t, uint64(10), runtime.config.bvbftConfig.EpochSize)
	assert.Equal(t, "0x0000000000000000000000000000000000000101", contracts.ValidatorSetContract.String())
	assert.Equal(t, "0x1300000000000000000000000000000000000000", runtime.config.bvbftConfig.Bridge.StateSenderAddr.String())
	assert.Equal(t, "0x1000000000000000000000000000000000000000", runtime.config.bvbftConfig.Bridge.CheckpointManagerAddr.String())
	assert.True(t, runtime.IsBridgeEnabled())
	systemStateMock.AssertExpectations(t)
	blockchainMock.AssertExpectations(t)
	bvbftBackendMock.AssertExpectations(t)
}
...
func TestConsensusRuntime_calculateCommitEpochInput_SecondEpoch(t *testing.T) {
	...
	bvbftBackendMock := new(bvbftBackendMock)
	bvbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.GetPublicIdentities()).Twice()
