...
func initValidatorSet(bvbftConfig bvbftConfig, transition *state.Transition) error {
	initialValidators := make([]*contractsapi.ValidatorInit, len(bvbftConfig.InitialValidatorSet))
	for i, validator := range bvbftConfig.InitialValidatorSet {
	...
	initFn := &contractsapi.InitializeValidatorSetFn{
		...
		NewRootChainManager: bvbftConfig.Bridge.CustomSupernetManagerAddr,
		NewEpochSize:        new(big.Int).SetUint64(bvbftConfig.EpochSize),
...
func initRewardPool(bvbftConfig bvbftConfig, transition *state.Transition) error {
	initFn := &contractsapi.InitializeRewardPoolFn{
		NewRewardToken:  bvbftConfig.RewardConfig.TokenAddress,
		NewRewardWallet: bvbftConfig.RewardConfig.WalletAddress,
		NewValidatorSet: contracts.ValidatorSetContract,
		NewBaseReward:   new(big.Int).SetUint64(bvbftConfig.EpochReward),
	}
...
func mintRewardTokensToWallet(bvbftConfig bvbftConfig, transition *state.Transition) error {
	if isNativeRewardToken(bvbftConfig) {
	...
	mintFn := contractsapi.MintRootERC20Fn{
		To:     bvbftConfig.RewardConfig.WalletAddress,
		Amount: bvbftConfig.RewardConfig.WalletAmount,
	}
	...
	return callContract(contracts.SystemCaller, bvbftConfig.RewardConfig.TokenAddress, input,
...
func approveRewardPoolAsSpender(bvbftConfig bvbftConfig, transition *state.Transition) error {
	...
	return callContract(bvbftConfig.RewardConfig.WalletAddress,
		bvbftConfig.RewardConfig.TokenAddress, input, "RewardToken.approve", transition)
}
...
func isNativeRewardToken(cfg bvbftConfig) bool {
