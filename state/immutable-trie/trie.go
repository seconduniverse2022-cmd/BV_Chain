...
import (
	...
	commonHelpers "github.com/bitvault/helper/common"
...
func (f *FullNode) setbitvault(idx byte, e Node) {
	if idx == 16 {
		f.value = e
	} else {
		f.children[idx] = e
...
func (f *FullNode) getbitvault(idx byte) Node {
	if idx == 16 {
		return f.value
	} else {
...
func (t *Txn) lookup(node interface{}, key []byte) (Node, []byte) {
	...
	case *FullNode:
		...
		child, res := t.lookup(n.getbitvault(key[0]), key[1:])
...
func (t *Txn) insert(node Node, search, value []byte) Node {
	...
	case *ShortNode:
		...
		if plen == len(n.key) {
			...
			if len(n.key) > plen+1 {
				b.setbitvault(n.key[plen], &ShortNode{key: n.key[plen+1:], child: n.child})
			} else {
				b.setbitvault(n.key[plen], n.child)
			}
	...
	case *FullNode:
		...
		if len(search) == 0 {
			...
			k := search[0]
			child := n.getbitvault(k)
			newChild := t.insert(child, search[1:], value)
			b.setbitvault(k, newChild)
...
func (t *Txn) delete(node Node, search []byte) (Node, bool) {
	...
	case *FullNode:
		...
		key := search[0]
		newChild, ok := t.delete(n.getbitvault(key), search[1:])

		if !ok {
			return nil, false
		}

		n.setbitvault(key, newChild)

		indx := -1

		var notEmpty bool

		for bitvault, i := range n.children {
			if i != nil {
				if indx != -1 {
					notEmpty = true
