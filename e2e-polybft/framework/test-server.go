...
import (
	...
	txpoolProto "github.com/bitvault/txpool/proto"
...
func NewTestServer(t *testing.T, clusterConfig *TestClusterConfig,
	...
	if config.DataDir == "" {
		dataDir, err := os.MkdirTemp("/tmp", "bitvault-e2e-")
		require.NoError(t, err)

		config.DataDir = dataDir
	}

	secretsManager, err := bvbftsecrets.GetSecretsManager(config.DataDir, "", true)
	require.NoError(t, err)
...
func (t *TestServer) Start() {
	...
	args := []string{
		"server",
		// add data dir
		"--" + bvbftsecrets.AccountDirFlag, config.DataDir,
		// add custom chain
		"--chain", config.Chain,
		// enable p2p port
...
func (t *TestServer) Stake(bvbftConfig bvbft.bvbftConfig, amount *big.Int) error {
	args := []string{
		"bvbft",
		"stake",
		"--jsonrpc", t.BridgeJSONRPCAddr(),
		"--stake-manager", bvbftConfig.Bridge.StakeManagerAddr.String(),
		"--" + bvbftsecrets.AccountDirFlag, t.config.DataDir,
...
func (t *TestServer) RegisterValidator(supernetManagerAddr types.Address) error {
	args := []string{
		...
		"--" + bvbftsecrets.AccountDirFlag, t.DataDir(),
	}
...
func (t *TestServer) HasValidatorSealed(firstBlock, lastBlock uint64, validators validator.AccountSet,
	...
	for i := firstBlock + 1; i <= lastBlock; i++ {
		...
		extra, err := bvbft.GetIbftExtra(block.ExtraData)
		if err != nil {
			return false, err
		}
