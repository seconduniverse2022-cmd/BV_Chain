...
import (
	...
	bridgeCommon "github.com/bitvault/command/bridge/common"
...
func (t *TestBridge) deployRootchainContracts(genesisPath string) error {
	bvbftConfig, err := bvbft.LoadbvbftConfig(genesisPath)
	if err != nil {
		return err
	}
	...
	args := []string{
		"rootchain",
		"deploy",
		"--stake-manager", bvbftConfig.Bridge.StakeManagerAddr.String(),
		"--stake-token", bvbftConfig.Bridge.StakeTokenAddr.String(),
...
func (t *TestBridge) fundAddressesOnRoot(tokenConfig *bvbft.TokenConfig, bvbftConfig bvbft.bvbftConfig) error {
	...
	balances := make([]*big.Int, len(bvbftConfig.InitialValidatorSet))
	...
	if err := t.FundValidators(bvbftConfig.Bridge.StakeTokenAddr,
...
func (t *TestBridge) whitelistValidators(validatorAddresses []types.Address,
	bvbftConfig bvbft.bvbftConfig) error {
	...
	args := []string{
		...
		"--supernet-manager", bvbftConfig.Bridge.CustomSupernetManagerAddr.String(),
...
func (t *TestBridge) registerGenesisValidators(bvbftConfig bvbft.bvbftConfig) error {
	validatorSecrets, err := genesis.GetValidatorKeyFiles(t.clusterConfig.TmpDir, t.clusterConfig.ValidatorPrefix)
	if err != nil {
		...
		g.Go(func() error {
			...
			default:
				args := []string{
					...
					"--jsonrpc", t.JSONRPCAddr(),
					"--supernet-manager", bvbftConfig.Bridge.CustomSupernetManagerAddr.String(),
					"--" + bvbftsecrets.AccountDirFlag, path.Join(t.clusterConfig.TmpDir, secret),
				}
...
func (t *TestBridge) initialStakingOfGenesisValidators(bvbftConfig bvbft.bvbftConfig) error {
	validatorSecrets, err := genesis.GetValidatorKeyFiles(t.clusterConfig.TmpDir, t.clusterConfig.ValidatorPrefix)
	if err != nil {
		...
		g.Go(func() error {
			...
			default:
				args := []string{
					...
					"--supernet-id", strconv.FormatInt(bvbftConfig.SupernetID, 10),
					"--stake-token", bvbftConfig.Bridge.StakeTokenAddr.String(),
				}
...
func (t *TestBridge) finalizeGenesis(genesisPath string, bvbftConfig bvbft.bvbftConfig) error {
	args := []string{
		"bvbft",
		"supernet",
		"--jsonrpc", t.JSONRPCAddr(),
...
func (t *TestBridge) FundValidators(tokenAddress types.Address, secretsPaths []string, amounts []*big.Int) error {
	...
	for i := 0; i < len(secretsPaths); i++ {
		secretsManager, err := bvbftsecrets.GetSecretsManager(secretsPaths[i], "", true)
		if err != nil {
			return err
		}
...
func (t *TestBridge) mintNativeRootToken(validatorAddresses []types.Address, tokenConfig *bvbft.TokenConfig,
	bvbftConfig bvbft.bvbftConfig) error {
	...
	args := []string{
		...
		"mint-erc20",
		"--jsonrpc", t.JSONRPCAddr(),
		"--erc20-token", bvbftConfig.Bridge.RootNativeERC20Addr.String(),
	}
...
func (t *TestBridge) premineNativeRootToken(tokenConfig *bvbft.TokenConfig,
	bvbftConfig bvbft.bvbftConfig) error {
	if tokenConfig.IsMintable {
		// if token is mintable, it is premined in genesis command,
		// so we just return here
	...
	premineCmdArgs := func(secret, key string, amount *big.Int) error {
		args := []string{
			...
			"--erc20-token", bvbftConfig.Bridge.RootNativeERC20Addr.String(),
			"--root-erc20-predicate", bvbftConfig.Bridge.RootERC20PredicateAddr.String(),
		}

		if secret != "" {
			args = append(args, "--"+bvbftsecrets.AccountDirFlag, path.Join(t.clusterConfig.TmpDir, secret))
		} else {
