...
const (
	...
	NativeTokenMintableTestCfg = "Mintable bitvault Coin:MEC:18:true:%s" //nolint:gosec
)
...
type TestClusterConfig struct {
	...
	BurnContract         *bvbft.BurnContractInfo
...
func WithBurnContract(burnContract *bvbft.BurnContractInfo) ClusterOption {
	return func(h *TestClusterConfig) {
...
func NewTestCluster(t *testing.T, validatorsCount int, opts ...ClusterOption) *TestCluster {
	...
	config.TmpDir, err = os.MkdirTemp("/tmp", "e2e-bvbft-")
...

	{
		...
		tokenConfig, err := bvbft.ParseRawTokenConfig(cluster.Config.NativeTokenConfigRaw)
		require.NoError(t, err)
	...
	err = cluster.Bridge.fundAddressesOnRoot(tokenConfig, bvbftConfig)
	require.NoError(t, err)

	// whitelist genesis validators on the rootchain
	err = cluster.Bridge.whitelistValidators(addresses, bvbftConfig)
	require.NoError(t, err)

	// register genesis validators on the rootchain
	err = cluster.Bridge.registerGenesisValidators(bvbftConfig)
	require.NoError(t, err)

	// do initial staking for genesis validators on the rootchain
	err = cluster.Bridge.initialStakingOfGenesisValidators(bvbftConfig)
	require.NoError(t, err)

	// add premine if token is non-mintable
	err = cluster.Bridge.mintNativeRootToken(addresses, tokenConfig, bvbftConfig)
	require.NoError(t, err)

	err = cluster.Bridge.premineNativeRootToken(tokenConfig, bvbftConfig)
	require.NoError(t, err)

	// finalize genesis validators on the rootchain
	err = cluster.Bridge.finalizeGenesis(genesisPath, bvbftConfig)
	require.NoError(t, err)
...
// RunbitvaultCommand - calls a command line bitvault function
func RunbitvaultCommand(args []string, stdout io.Writer) error {
...
func (c *TestCluster) InitSecrets(prefix string, count int) ([]types.Address, error) {
	...
	stdOut := c.Config.GetStdout("bvbft-secrets", &b)
