...
import (
	...
	"github.com/bitvault/command/sidechain"
	"github.com/bitvault/consensus/bvbft"
	"github.com/bitvault/consensus/bvbft/contractsapi"
	"github.com/bitvault/contracts"
	"github.com/bitvault/e2e-bvbft/framework"
	helperCommon "github.com/bitvault/helper/common"
	"github.com/bitvault/state/runtime/addresslist"
...
func TestE2E_Bridge_Transfers(t *testing.T) {
	...
	bvbftCfg, err := bvbft.LoadbvbftConfig(path.Join(cluster.Config.TmpDir, chainConfigFileName))
	require.NoError(t, err)
	...
	require.NoError(t, cluster.Bridge.Deposit(
		common.ERC20,
		bvbftCfg.Bridge.RootNativeERC20Addr,
		bvbftCfg.Bridge.RootERC20PredicateAddr,
	...
	t.Run("bridge ERC 20 tokens", func(t *testing.T) {
		...
		currentExtra, err := bvbft.GetIbftExtra(currentBlock.ExtraData)
		require.NoError(t, err)
		...
		exitHelper := bvbftCfg.Bridge.ExitHelperAddr
		...
		for i := 0; i < numberOfAttempts; i++ {
			...
			require.NoError(t, waitForRootchainEpoch(currentEpoch+uint64(i), 3*time.Minute,
				rootchainTxRelayer, bvbftCfg.Bridge.CheckpointManagerAddr))
		...
		for _, receiver := range receivers {
			balance := erc20BalanceOf(t, types.StringToAddress(receiver),
				bvbftCfg.Bridge.RootNativeERC20Addr, rootchainTxRelayer)
			require.Equal(t, big.NewInt(amount), balance)
		}
	})
	...
	t.Run("non native ERC20 deposit and withdraw", func(t *testing.T) {
		...
		require.NoError(t, waitForRootchainEpoch(
			...
			3*time.Minute,
			rootchainTxRelayer,
			bvbftCfg.Bridge.CheckpointManagerAddr,
		))
...
func TestE2E_Bridge_ERC721Transfer(t *testing.T) {
	...
	require.NoError(
		...
		cluster.Bridge.Deposit(
			...
			bvbftCfg.Bridge.RootERC721PredicateAddr,
			rootHelper.TestAccountPrivKey,
			strings.Join(receivers[:], ","),
			"",
	...
	l1ChildTokenAddr := getChildToken(t, contractsapi.RootERC721Predicate.Abi, bvbftCfg.Bridge.RootERC721PredicateAddr,
	...
	currentEpoch := currentExtra.Checkpoint.EpochNumber
	require.NoError(t, waitForRootchainEpoch(currentEpoch, 3*time.Minute, rootchainTxRelayer, bvbftCfg.Bridge.CheckpointManagerAddr))
...
func TestE2E_Bridge_ERC1155Transfer(t *testing.T) {
	...
	require.NoError(
		...
		cluster.Bridge.Deposit(
			...
			bvbftCfg.Bridge.RootERC1155PredicateAddr,
			rootHelper.TestAccountPrivKey,
			strings.Join(receivers[:], ","),
			strings.Join(amounts[:], ","),
			strings.Join(tokenIDs[:], ","),
	...
	// retrieve child token address
	l1ChildTokenAddr := getChildToken(t, contractsapi.RootERC1155Predicate.Abi, bvbftCfg.Bridge.RootERC1155PredicateAddr,
...
func TestE2E_Bridge_ChildChainMintableTokensTransfer(t *testing.T) {
	...
	require.NoError(t, validatorSrv.RootchainFundFor(depositors, funds, bvbftCfg.Bridge.StakeTokenAddr))
	...
	t.Run("bridge native tokens", func(t *testing.T) {
		...
		extra, err := bvbft.GetIbftExtra(latestBlock.ExtraData)
		require.NoError(t, err)

		// wait for checkpoint to get submitted before invoking exit transactions
		require.NoError(t,
			waitForRootchainEpoch(extra.Checkpoint.EpochNumber, 2*time.Minute, rootchainTxRelayer, bvbftCfg.Bridge.CheckpointManagerAddr))
		...
		for exitEventID := uint64(1); exitEventID <= transfersCount+1; exitEventID++ {
			require.NoError(t,
				cluster.Bridge.SendExitTransaction(bvbftCfg.Bridge.ExitHelperAddr, exitEventID, validatorSrv.JSONRPCAddr()))
		}
		...
		l1ChildToken := getChildToken(t, contractsapi.ChildMintableERC20Predicate.Abi, bvbftCfg.Bridge.ChildMintableERC20PredicateAddr,
		...
		for i, depositorKey := range depositorKeys {
			err = cluster.Bridge.Withdraw(
				...
				"",
				cluster.Bridge.JSONRPCAddr(),
				bvbftCfg.Bridge.ChildMintableERC20PredicateAddr,
				l1ChildToken,
				true)
			require.NoError(t, err)
		}
	...
	t.Run("bridge ERC 721 tokens", func(t *testing.T) {
		...
		childChainBlockExtra, err := bvbft.GetIbftExtra(childChainBlock.ExtraData)
		require.NoError(t, err)

		// wait for checkpoint to be submitted
		require.NoError(t,
			waitForRootchainEpoch(childChainBlockExtra.Checkpoint.EpochNumber, 2*time.Minute, rootchainTxRelayer, bvbftCfg.Bridge.CheckpointManagerAddr))
		...
		for i := initialExitEventID; i <= initialExitEventID+transfersCount; i++ {
			require.NoError(t,
				cluster.Bridge.SendExitTransaction(bvbftCfg.Bridge.ExitHelperAddr, i, validatorSrv.JSONRPCAddr()))
		}
		...
		l1ChildToken := getChildToken(t, contractsapi.ChildMintableERC721Predicate.Abi, bvbftCfg.Bridge.ChildMintableERC721PredicateAddr,
		...
		for i, depositorKey := range depositorKeys {
			err = cluster.Bridge.Withdraw(
				...
				fmt.Sprintf("%d", i),
				cluster.Bridge.JSONRPCAddr(),
				bvbftCfg.Bridge.ChildMintableERC721PredicateAddr,
				l1ChildToken,
				true)
			require.NoError(t, err)
		}
...
func TestE2E_CheckpointSubmission(t *testing.T) {
	...
	checkpointManagerAddr := ethgo.Address(bvbftCfg.Bridge.CheckpointManagerAddr)
...
func TestE2E_Bridge_ChangeVotingPower(t *testing.T) {
	...
	queryValidators := func(handler func(idx int, validatorInfo *bvbft.ValidatorInfo)) {
		for i, validatorAddr := range votingPowerChangeValidators {
			// query validator info
			validatorInfo, err := sidechain.GetValidatorInfo(
				validatorAddr,
				bvbftCfg.Bridge.CustomSupernetManagerAddr,
				bvbftCfg.Bridge.StakeManagerAddr,
				bvbftCfg.SupernetID,
				rootRelayer,
				childRelayer)
			require.NoError(t, err)
	...
	validatorsMap := make(map[ethgo.Address]*bvbft.ValidatorInfo, votingPowerChanges)

	queryValidators(func(idx int, validator *bvbft.ValidatorInfo) {
		...
		require.NoError(t, validatorSrv.RootchainFund(bvbftCfg.Bridge.StakeTokenAddr, validator.WithdrawableRewards))

		// stake previously funded amount
		require.NoError(t, validatorSrv.Stake(bvbftCfg, validator.WithdrawableRewards))
	})
	...
	require.NoError(t, waitForRootchainEpoch(targetEpoch, 2*time.Minute,
		rootRelayer, bvbftCfg.Bridge.CheckpointManagerAddr))

	// make sure that correct validator set is submitted to the checkpoint manager
	checkpointValidators, err := getCheckpointManagerValidators(rootRelayer, ethgo.Address(bvbftCfg.Bridge.CheckpointManagerAddr))
	require.NoError(t, err)
...
func TestE2E_Bridge_Transfers_AccessLists(t *testing.T) {
	...
	require.NoError(t, cluster.Servers[0].RootchainFundFor([]types.Address{adminAddr}, []*big.Int{ethgo.Ether(10)},
		bvbftCfg.Bridge.StakeTokenAddr))
	...
	t.Run("bridge native (ERC 20) tokens", func(t *testing.T) {
		...
		for _, receiver := range receivers {
			balance := erc20BalanceOf(t, types.StringToAddress(receiver), bvbftCfg.Bridge.RootNativeERC20Addr, rootchainTxRelayer)
...
func TestE2E_Bridge_NonMintableERC20Token_WithPremine(t *testing.T) {
	...
	cluster := framework.NewTestCluster(t, 5,
		...
		framework.WithBurnContract(&bvbft.BurnContractInfo{
			BlockNumber: 0,
