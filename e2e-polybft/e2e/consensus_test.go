...
func TestE2E_Consensus_RegisterValidator(t *testing.T) {
	...
	bvbftConfig, err := bvbft.LoadbvbftConfig(path.Join(cluster.Config.TmpDir, chainConfigFileName))
	require.NoError(t, err)
	...
	_, err = bvbft.GetIbftExtra(genesisBlock.ExtraData)
	require.NoError(t, err)
	...
	require.NoError(t, firstValidator.RegisterValidator(bvbftConfig.Bridge.CustomSupernetManagerAddr))

	// register the second validator without stake
	require.NoError(t, secondValidator.RegisterValidator(bvbftConfig.Bridge.CustomSupernetManagerAddr))

	// stake manually for the first validator
	require.NoError(t, firstValidator.Stake(bvbftConfig, initialStake))

	// stake manually for the second validator
	require.NoError(t, secondValidator.Stake(bvbftConfig, initialStake))

	firstValidatorInfo, err := sidechain.GetValidatorInfo(firstValidatorAddr,
		bvbftConfig.Bridge.CustomSupernetManagerAddr, bvbftConfig.Bridge.StakeManagerAddr,
		bvbftConfig.SupernetID, rootChainRelayer, childChainRelayer)
	require.NoError(t, err)
	...
	require.NoError(t, cluster.WaitForBlock(bvbftConfig.EpochSize*4, time.Minute))

	checkpointManagerAddr := ethgo.Address(bvbftConfig.Bridge.CheckpointManagerAddr)

	// check if the validators are added to active validator set
	rootchainValidators := []*bvbft.ValidatorInfo{}
	...
	require.NoError(t, cluster.WaitForBlock(currentBlock.Number+(bvbftConfig.EpochSize*2), time.Minute))
...
func TestE2E_Consensus_Validator_Unstake(t *testing.T) {
	...
	require.NoError(t, cluster.WaitForBlock(bvbftCfg.EpochSize*3, time.Minute))

	validatorInfo, err := sidechain.GetValidatorInfo(validatorAddr,
		bvbftCfg.Bridge.CustomSupernetManagerAddr, bvbftCfg.Bridge.StakeManagerAddr,
		bvbftCfg.SupernetID, rootChainRelayer, childChainRelayer)
	require.NoError(t, err)
	...
	require.NoError(t, cluster.WaitForBlock(bvbftCfg.EpochSize*4, time.Minute))
	...
	require.NoError(t, waitForRootchainEpoch(currentEpoch, time.Minute,
		rootChainRelayer, bvbftCfg.Bridge.CheckpointManagerAddr))

	exitEventID := uint64(1)

	// send exit transaction to exit helper
	err = cluster.Bridge.SendExitTransaction(bvbftCfg.Bridge.ExitHelperAddr, exitEventID, srv.JSONRPCAddr())
	require.NoError(t, err)
...
func TestE2E_Consensus_MintableERC20NativeToken(t *testing.T) {
	const (
		...
		tokenName   = "bitvault Coin"
		tokenSymbol = "bitvault"
		decimals    = uint8(5)
	)
...
func TestE2E_Consensus_EIP1559Check(t *testing.T) {
	...
	cluster := framework.NewTestCluster(t, 5,
		...
		framework.WithBurnContract(&bvbft.BurnContractInfo{BlockNumber: 0, Address: types.ZeroAddress}),
	)
	defer cluster.Stop()
