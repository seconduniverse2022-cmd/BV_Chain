...
import (
	...
	frameworkbvbft "github.com/bitvault/e2e-bvbft/framework"
	"github.com/bitvault/e2e/framework"
...
func TestE2E_Migration(t *testing.T) {
	...
	err = frameworkbvbft.RunbitvaultCommand([]string{
		"regenesis",
	...
	cluster := frameworkbvbft.NewTestCluster(t, 7,
		frameworkbvbft.WithNonValidators(2),
		frameworkbvbft.WithValidatorSnapshot(5),
		frameworkbvbft.WithTestRewardToken(),
		frameworkbvbft.WithGenesisState(tmpDir, types.Hash(stateRoot)),
	)
	defer cluster.Stop()
	...
	cluster.InitTestServer(t, "test-chain-8", cluster.Bridge.JSONRPCAddr(), frameworkbvbft.None)
