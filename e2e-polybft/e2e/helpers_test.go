...
func getCheckpointManagerValidators(relayer txrelayer.TxRelayer, checkpointManagerAddr ethgo.Address) ([]*bvbft.ValidatorInfo, error) {
	...
	validators := make([]*bvbft.ValidatorInfo, validatorsCount)
	...
	for i := 0; i < int(validatorsCount); i++ {
		...
		validators[i] = &bvbft.ValidatorInfo{
