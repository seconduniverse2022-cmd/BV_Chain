...
bitvault_bitvault_BIN=./bitvault
CHAIN_CUSTOM_OPTIONS=$(tr "\n" " " << EOL
--block-gas-limit 10000000
--epoch-size 10
--chain-id 51001
--name bitvault-docker
...
createGenesisConfig() {
  ...
  "$bitvault_bitvault_BIN" genesis $CHAIN_CUSTOM_OPTIONS \
...
case "$1" in
      ...
      case "$2" in 
          "ibft")
              if [ -f "$GENESIS_PATH" ]; then
                  ...
                  secrets=$("$bitvault_bitvault_BIN" secrets init --insecure --num 4 --data-dir /data/data- --json)
              ...
              ;;
          "bvbft")
              echo "Generating bvbft secrets..."
              secrets=$("$bitvault_bitvault_BIN" bvbft-secrets init --insecure --num 4 --data-dir /data/data- --json)
              echo "Secrets have been successfully generated"
              ...
              createGenesisConfig "$2" "$secrets" \
                ...
                --native-token-config "bitvault:MATIC:18:true:$(echo "$secrets" | jq -r '.[0] | .address')" \
                --proxy-contracts-admin ${proxyContractsAdmin}

              echo "Deploying stake manager..."
              "$bitvault_bitvault_BIN" bvbft stake-manager-deploy \
                --jsonrpc http://rootchain:8545 \
                --genesis /data/genesis.json \
                --proxy-contracts-admin ${proxyContractsAdmin} \
                --test

              stakeManagerAddr=$(cat /data/genesis.json | jq -r '.params.engine.bvbft.bridge.stakeManagerAddr')
              stakeToken=$(cat /data/genesis.json | jq -r '.params.engine.bvbft.bridge.stakeTokenAddr')

              "$bitvault_bitvault_BIN" rootchain deploy \
              ...
              customSupernetManagerAddr=$(cat /data/genesis.json | jq -r '.params.engine.bvbft.bridge.customSupernetManagerAddr')
              supernetID=$(cat /data/genesis.json | jq -r '.params.engine.bvbft.supernetID')
              ...
              "$bitvault_bitvault_BIN" rootchain fund \
                --json-rpc http://rootchain:8545 \
                --stake-token ${stakeToken} \
                --mint \
                --addresses ${addresses} \
              ...
              "$bitvault_bitvault_BIN" bvbft whitelist-validators \
                --addresses ${addresses} \
              ...
              while [ $counter -le 4 ]; do
                ...
                "$bitvault_bitvault_BIN" bvbft register-validator \
                  --supernet-manager ${customSupernetManagerAddr} \
                  --data-dir /data/data-${counter} \
                  --jsonrpc http://rootchain:8545

                "$bitvault_bitvault_BIN" bvbft stake \
                  --data-dir /data/data-${counter} \
              ...
              "$bitvault_bitvault_BIN" bvbft supernet \
                --private-key aa75e9a7d427efc732f8e4f1a5b7646adcc61fd5bae40f80d13c8419c9f43d6d \
                --supernet-manager ${customSupernetManagerAddr} \
  ...
  "start-node-1")
    relayer_flag=""
    # Start relayer only when run in bvbft
    if [ "$2" == "bvbft" ]; then
      echo "Starting relayer..."
      relayer_flag="--relayer"
    fi

    "$bitvault_bitvault_BIN" server \
   ...
   ;;
   *)
      echo "Executing bitvault..."
      exec "$bitvault_bitvault_BIN" "$@"
      ;;
esac
