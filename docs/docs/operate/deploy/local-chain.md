In this section, we'll prepare initiate a new chain with bvbft consensus and prepare the initial bitvault nodes.

## 1. Generate Keys

To initialize bvbft consensus, we need to generate the necessary secrets for each node.

The `bitvault bvbft-secrets` command is used to generate account secrets for validators. The command initializes private keys for the consensus client (validators + networking) to a Secrets Manager config file.
...
| `--data-dir`    | The directory for the bitvault data if the local FS is used.                                          | `--data-dir /path/to/dir`  |
...
Next, create a config.json file within your bitvault directory with the following contents:
...
To generate the necessary secrets for your validator node, run the bvbft-secrets command. In your terminal, input:

```bash
go run main.go bvbft-secrets --config config.json
```
...
> The secrets output can be retrieved again if needed by running the following command: `./bitvault bvbft-secrets --data-dir test-chain-X/`
