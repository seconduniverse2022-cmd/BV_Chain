...
<div align="center">
  <img src="/img/bitvault/bridge-withdraw-rootchain.excalidraw.png" alt="bridge" width="100%" height="30%" />
</div>

### Withdrawing from the Childchain

<div align="center">
  <img src="/img/bitvault/bridge-withdraw-childchain.excalidraw.png" alt="bridge" width="100%" height="30%" />
</div>

For a detailed understanding of how bridging works, please refer to the sequence diagrams available [<ins>here</ins>](https://github.com/bitvault/blob/develop/docs/bridge/sequences.md).
...
This command withdraws ERC-20 tokens from an bitvault-powered chain to a rootchain.
...
- Replace `child_erc20_predicate_address` with the address of the ERC-20 predicate contract on the bitvault-powered chain.
- Replace `child_erc20_token_address` with the address of the ERC-20 token contract on the bitvault-powered chain (optional, only required if the token is not a default ERC-20 token).
- Replace `child_chain_json_rpc_endpoint` with the JSON-RPC endpoint of the bitvault-powered chain.

```bash
 ./bitvault bridge withdraw-erc20 \
...
- The address of the ERC-20 predicate contract on the bitvault-powered chain is `0x3456789abcdef0123456789abcdef0123456789`.
- The address of the ERC-20 token contract on the bitvault-powered chain is `0x456789abcdef0123456789abcdef0123456789`.
- The JSON-RPC endpoint for the bitvault-powered chain is `http://json-rpc-endpoint.com:8545`.
...
This command sends an exit transaction to the `ExitHelper` contract on the rootchain for a token that was deposited on the bitvault-powered chain. It basically finalizes a withdrawal (initiated on the bitvault-powered chain) and transfers assets to receiving address on a rootchain.
...
In this example, we're sending an exit transaction on a test bitvault instance:
...
- The JSON-RPC endpoint for the rootchain is `http://root-chain-json-rpc-endpoint.com:8545`, and the JSON-RPC endpoint for the bitvault-powered chain is `http://json-rpc-endpoint.com:8545`.
...
This command withdraws ERC-721 tokens from an bitvault-powered chain to a rootchain.
...
- Replace `child_predicate_address` with the address of the predicate contract on the bitvault-powered chain that holds the tokens being withdrawn.
- Replace `child_token_address` with the address of the ERC-721 token contract on the bitvault-powered chain that holds the tokens being withdrawn.
- Replace `child_chain_json_rpc_endpoint` with the JSON-RPC endpoint of the bitvault-powered chain.

```bash
./bitvault bridge withdraw-erc721 \
...
In this example, we're withdrawing ERC-721 tokens from a test bitvault instance:

- We're using a private key of `0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef`.
- We're withdrawing tokens with IDs `123` and `456`.
- The address of the ERC-721 bitvault-powered chain predicate contract is `0x23456789abcdef0123456789abcdef012345678`.
- The address of the ERC-721 bitvault-powered chain token contract is `0x3456789abcdef0123456789abcdef012345678`.
- The JSON-RPC endpoint for the bitvault-powered chain is `http://json-rpc-endpoint.com:8545`.
...
This command withdraws ERC-1155 tokens from the bitvault-powered chain to the rootchain.
...
```bash
./bitvault bridge withdraw-erc1155 \
...
In this example, we're withdrawing ERC-1155 tokens from a test bitvault instance to the rootchain:
