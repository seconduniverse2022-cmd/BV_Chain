This serves as an index for the bitvault deployment guides. 
The guides explain the end-to-end process for setting up and deploying a local bitvault-powered chain.
...
In this tutorial, you will learn how to set up and initialize an bitvault-powered chain with multiple nodes. You will learn the complete end-to-end genesis workflow of building an bitvault-powered chain, including:

- Generating private keys for bvbft nodes.
...
- Running an (bitvault) cluster consisting of multiple bvbft nodes.

By the end of this tutorial, you will have a fully functional bitvault test network that can be used to process transactions with high throughput and low latency.

## What you'll do

The deployment guides will cover the steps outlined below. If you are deploying an bitvault-powered chain for the first time, please navigate each section in order. 

1. [<ins>Spawn a New bitvault-powered chain</ins>](local-chain.md)
2. [<ins>Configure Your New bitvault-powered chain</ins>](genesis.md)
3. [<ins>Configure the Rootchain</ins>](rootchain-config.md)
4. [<ins>Configure the Initial Validator Set</ins>](genesis-validators.md)
5. [<ins>Start Your New bitvault-powered chain</ins>](start-chain.md)

<details>
<summary>Fast-track guide ↓</summary>

**Here's the fast-track guide if you're looking for a quick guide on the essential commands needed to set up a local bitvault-powered chain.**

1. Init secrets:

   ```bash
   ./bitvault bvbft-secrets --data-dir test-chain- --num 4
   ```
...
3. Deploy and initialize rootchain contracts:
   ...
   ```bash
   ./bitvault rootchain server
   ```

   ```bash
   ./bitvault rootchain deploy --deployer-key <hex_encoded_rootchain_account_private_key> [--genesis ./genesis.json] [--json-rpc http://127.0.0.1:8545] [--test]
   ```

4. Fund validators on rootchain:

   ```bash
   ./bitvault rootchain fund --data-dir ./test-chain-1
   ```

5. Allowlist validators on rootchain:

   ```bash
   ./bitvault bvbft whitelist-validators --private-key <hex_encoded_rootchain_account_private_key_of_supernetManager_deployer> --addresses <addresses_of_validators> --supernet-manager <address_of_SupernetManager_contract>
   ```

6. Register validators on rootchain:

   ```bash
   ./bitvault bvbft register-validator --data-dir ./test-chain-1 --supernet-manager <address_of_SupernetManager_contract>
   ```

7. Initial staking on rootchain:

   ```bash
   ./bitvault bvbft stake --data-dir ./test-chain-1 --chain-id <id_of_child_chain_from_genesis> --amount <amount_of_tokens_to_stake> --stake-manager <address_of_StakeManager_contract> --native-root-token <address_of_native_root_token>
   ```

8. Finalize genesis validator set on rootchain:

   ```bash
    ./bitvault bvbft supernet --private-key <hex_encoded_rootchain_account_private_key_of_supernetManager_deployer> \
...
9. Run (child chain) cluster:
    ...
    ```bash
    ./bitvault server --data-dir ./test-chain-1 --chain genesis.json --grpc-address :5001 --libp2p :30301 --jsonrpc :10001 \
    --seal --log-level DEBUG

    ./bitvault server --data-dir ./test-chain-2 --chain genesis.json --grpc-address :5002 --libp2p :30302 --jsonrpc :10002 \
    --seal --log-level DEBUG

    ./bitvault server --data-dir ./test-chain-3 --chain genesis.json --grpc-address :5003 --libp2p :30303 --jsonrpc :10003 \
    --seal --log-level DEBUG

    ./bitvault server --data-dir ./test-chain-4 --chain genesis.json --grpc-address :5004 --libp2p :30304 --jsonrpc :10004 \
    --seal --log-level DEBUG
    ```
