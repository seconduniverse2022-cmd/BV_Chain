...
The `bitvault` rootchain server command starts an ethereum/client-go container, which runs a new Geth node.
...
You should see output similar to the following, indicating that the rootchain server is now running:
  ...
  ```bash
  {"status":"Pulling from bitvault/go-ethereum-console","id":"latest"}
  {"status":"Digest: sha256:6aad124b6775b96d05c94850dcafde45911f7bb2b473328dc4a792b1ffb2bdb6"}
  {"status":"Status: Image is up to date for ghcr.io/bitvault/go-ethereum-console:latest"}
...
| `--data-dir`                 | The directory for the bitvault data if the local FS is used.              | `--data-dir test-chain-`                     |
...
To deploy the rootchain contracts, we use the `bitvault rootchain deploy` command. Before deployment, consider the following actions:
...
You can also create a rootchain wallet and fund the nodes by using `bitvault-cli`.
Follow the steps outlined [<ins>here</ins>](https://github.com/maticnetwork/bitvault-cli).
...
| `--data-dir`    | Directory for the bitvault data if the local FS is used                                            |         |
...
Before deploying validator nodes on the bitvault-powered chain, we need to ensure that the validators have sufficient funds on the rootchain network. It's crucial to have enough funds in the validator account, as they need to cover the gas fees associated with their transactions on the rootchain.

To fund the validators' accounts on the rootchain, we use the `bitvault rootchain fund` command. When executed with the appropriate flags, it will:
...
Here's an example of how to fund a validator account (replace address value with your own):
  ...
  ```bash
  ./bitvault rootchain fund --addresses 0x77C1eedFf656477462ce16084fE5Dc7F8a2507B9 --amounts 1000000000000000000
  ```
...
Now that we have successfully configured the initial setup of your bitvault-powered chain, including both the childchain and rootchain, we can proceed to the next crucial step: rootchain staking and allowlisting through the configuration of the initial validator set.
