...
First, check your validator information by running the `bitvault bvbft validator-info` command.
...
| `--data-dir` | The directory for the bitvault data if the local FS is used | `/path/to/data/dir` |
...
```bash
./bitvault bvbft validator-info --data-dir ./test-chain-1
```
...
To unstake, use the `bitvault bvbft unstake` command.
...
| --data-dir          | Directory for the bitvault data if the local FS is used | --data-dir /path/to/data/dir          |
| --jsonrpc           | JSON-RPC interface                                         | --jsonrpc 0.0.0.0:8545                |

</details>

```bash
./bitvault bvbft unstake \
...
Once you have successfully unstaked, you will need to withdraw your unstaked tokens from the bitvault-powered chain to the rootchain. To do so, use the `bitvault bvbft withdraw-child` command.
...
| `--data-dir` | The directory for the bitvault data if the local FS is used | `~/bitvault/data` |
| `--jsonrpc` | The JSON-RPC interface (default "0.0.0.0:8545") | `127.0.0.1:8545` |

</details>

```bash
./bitvault bvbft withdraw-child \
    --account-dir <DATA_DIR> \
    --jsonrpc <JSONRPC_ADDR>
```
...
Once the exit event has been included in a checkpoint, you can send an exit transaction to execute the transaction on the rootchain. To do so, use the `bitvault bridge exit` command.
...
Finally, you can withdraw your tokens from the rootchain to your address by using the `bitvault bvbft withdraw-root` command.
...
| --data-dir             | directory for the bitvault data     | --data-dir /path/to/data/dir       |
| --jsonrpc              | JSON-RPC interface                       | --jsonrpc 0.0.0.0:8545             |
