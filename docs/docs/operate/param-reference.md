...
Configuration parameters are crucial for setting up and operating a bitvault-powered chain. You can configure these parameters using the genesis and server commands. Before running these commands, it is essential to generate keys using the bvbft-secrets command.

For information on the available configuration flags and their descriptions for customizing the genesis configuration of an bitvault-powered chain, refer to the categorized tabs below.

<Tabs
defaultValue="bvbft"
values={[
{ label: 'bvbft', value: 'bvbft', },
...
## bvbft Configuration Parameter Reference
...
| `--data-dir string`   | The directory for the bitvault data if the local FS is used   | ""            | YES       |
...
- `--config` and `--data-dir`: These flags are mutually exclusive. Use either `--config` to specify the path to the SecretsManager config file or `--data-dir` to set the directory for the bitvault data if the local FS is used.
...
| `--consensus` | The consensus protocol to be used | "bvbft" | NO | `genesis --consensus bvbft` | NO |
...
| `--name` | The name for the chain | "bitvault" | NO | `genesis --name "test-chain"` | NO |
...
| `--validators-path` | Root path containing bvbft validators' secrets | "./" | NO | `genesis --validators-path "/data/validators"` | NO |
...
- `--validators-path`: Root path containing bvbft validators' secrets. If `--validators` flag is not specified, validators' configuration will be read from this path.
- `--validators-prefix`: Folder prefix names for bvbft validators' secrets. If `--validators` flag is set, this prefix will be used for folder names.
...
| `--data-dir` string | The data directory used for storing bitvault client data. | “” | YES | Command: server Flag:--data-dir “./test-chain-1” | NO |
...
| `--log-to` string | Write all logs to the file at specified location instead of writing them to console. | “” | NO | Command: server Flag: --log-to “bitvault-log.log” | NO |
