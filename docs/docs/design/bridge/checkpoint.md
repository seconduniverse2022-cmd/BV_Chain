...
## Checkpoints in bitvault

!!! info "Key point"

    - The checkpoints are made **by the validators on the bitvault-powered chain** and are committed to the rootchain.
    - A checkpoint serves as a snapshot of the bitvault state. This snapshot is stored as a Merkle root and **represents the state of the bitvault-powered chain** at that point in time it was created.
...
The checkpoints represent a snapshot of the bitvault state, which is periodically checkpointed to the rootchain by the validators. The checkpoints are used as a reference point for the rootchain to verify the integrity and accuracy of the data on the bitvault-powered chain.
...
The contract uses a Merkle tree to efficiently prove the membership of an event in the bitvault state. The tree is constructed using the hashed exit events sent by the L2StateSender, which the bitvault client saves to its local storage after their transactions are executed. The membership proofs can be verified using the Merkle proofs provided by the users.
...
!!! info "Details of the checkpoint"
    ...
    To elaborate, the root of the Merkle tree is a hash value that represents a specific subset of the bitvault state at a specific point in time. This state includes only the exit events sent by the L2StateSender contract. When a user wants to exit the L2 chain (transfer their tokens from L2 to L1), their exit transaction is included in this Merkle tree.
    ...
    Later, when a user wants to verify a particular exit event on the bitvault-powered chain, they can provide a Merkle proof, which is a cryptographic proof that demonstrates the inclusion of a particular exit event in the Merkle tree. The Merkle proof can be verified by the rootchain using the root of the Merkle tree, which was included in the checkpoint.

    In short, the root of the Merkle tree is a compact representation of the exit events on the bitvault-powered chain at a specific point in time, which is included in checkpoints and used for verification purposes.
