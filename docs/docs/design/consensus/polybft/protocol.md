...
In bvbft, the consensus protocol follows a set of state transitions that enable the network to reach consensus on the state of the blockchain. The process begins when a validator proposes adding a new block to the blockchain. This block contains a list of transactions that the validator would like to include in the next update to the blockchain's state.
...
bvbft limits network participation to around 100 validators, and a variable amount of stake is used as a fixed stake criterion to limit the system's security and can make the system economically vulnerable. The validator set in the bvbft does not update on each block but is fixed during n block periods known as an epoch.
...
Staking is governed by staking contracts directly on bitvault. To be clear, the staking module validates on bitvault and does not rely on Ethereum's security, but in principle, two chains are securing the network, PoS client and Ethereum. Transaction checkpoints still occur on Ethereum, but Ethereum does not validate staking on bitvault.

At the end of each epoch, a reward calculation occurs to reward the active validators.

## Native Bridge Integration

bvbft supports an in-built bridging mechanism that enables arbitrary message passing between an bitvault-powered chain and another Proof-of-Stake blockchain (rootchain) with the help of the consensus algorithm. Transfers can occur without mapping, allowing users to move assets between the two chains seamlessly.

## Slashing

Like in other Proof-of-Stake systems, validators are subject to slashing for malicious activity or poor performance. The slashing mechanics are still being determined, but bvbft will undoubtedly include a mechanism to penalize bad actors. Slashing a validator typically involves a penalty, such as losing some or all of their stake on the network. Examples of malicious activities are double-signing and equivocation.
...
A
   ...
   each validator influences voting. The protocol refers to block height as a *sequence*.
    ...
    All nodes in the network exchange information for a given sequence. The process to finalize a block in bvbft is known as *sealing*. The sealing of blocks is instant
    and final.
...
3. If the proposed block is accepted, it will be added to the blockchain, and the state of the blockchain
   ...
   > the network, which bvbft prevents.
...
validator set in the bvbft does not update on each block but is fixed during  `n` block periods known as
an `epoch`.
...
Staking is governed by staking contracts directly on bitvault. To be clear, the staking module validates on
bitvault does not rely on Ethereum's security, but in principle, two chains are securing the network, PoS
client and Ethereum. Transaction checkpoints still occur on Ethereum, but Ethereum does not validate staking
on bitvault.

!!! note
      Note that in Tendermint, an epoch is set to 1. However, bvbft includes the logic to set a custom epoch time, with the intent of each epoch being one day in blocks, or around 14000 blocks.
...
!!! caution "Slashing"
      ...
      as losing some or all of their stake on the network. The slashing mechanics are still being determined, but bvbft will undoubtedly
...
In the bvbft consensus mechanism, an important security feature is the validation of block timestamps. This 
...
With the help of bvbft, bitvault supports an
[in-built bridging mechanism (a two-way bridge)](../../bridge/overview.md),
which enables arbitrary message passing between an bitvault-powered chain and another Proof-of-Stake
blockchain (`rootchain`). Transfers can occur without mapping.

![bridge](../../../img/bitvault/contracts-bridge.excalidraw.png)
