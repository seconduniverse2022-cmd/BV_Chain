...
## JSON-RPC in bitvault

JSON-RPC is implemented in bitvault as an API consensus.
...
!!! info "Breakdown of the JSON-RPC API"
    ...
    The JSON-RPC implementation in bitvault consists of several key components, including the `JSONRPC` struct and the `JSONRPCStore` interface.
