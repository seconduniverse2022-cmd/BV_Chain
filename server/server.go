...
import (
	...
	"github.com/bitvault/blockchain/storage"
	"github.com/bitvault/blockchain/storage/leveldb"
	"github.com/bitvault/blockchain/storage/memory"
	consensusbvbft "github.com/bitvault/consensus/bvbft"
	"github.com/bitvault/forkmanager"
	"github.com/bitvault/gasprice"

	"github.com/bitvault/archive"
	"github.com/bitvault/blockchain"
	"github.com/bitvault/chain"
	"github.com/bitvault/consensus"
	"github.com/bitvault/contracts"
	"github.com/bitvault/crypto"
	"github.com/bitvault/helper/common"
	"github.com/bitvault/helper/progress"
	"github.com/bitvault/jsonrpc"
	"github.com/bitvault/network"
	"github.com/bitvault/secrets"
	"github.com/bitvault/server/proto"
	"github.com/bitvault/state"
	itrie "github.com/bitvault/state/immutable-trie"
	"github.com/bitvault/state/runtime"
	"github.com/bitvault/state/runtime/addresslist"
	"github.com/bitvault/state/runtime/tracer"
	"github.com/bitvault/txpool"
	"github.com/bitvault/types"
	"github.com/bitvault/validate"
...
func NewServer(config *Config) (*Server, error) {
	...
	if ConsensusType(engineName) == bvbftConsensus {
		bvbftConfig, err := consensusbvbft.GetbvbftConfig(config.Chain)
		if err != nil {
			return nil, err
		}

		if bvbftConfig.InitialTrieRoot != types.ZeroHash {
			checkedInitialTrieRoot, err := itrie.HashChecker(bvbftConfig.InitialTrieRoot.Bytes(), stateStorage)
			if err != nil {
				return nil, fmt.Errorf("error on state root verification %w", err)
			}

			if checkedInitialTrieRoot != bvbftConfig.InitialTrieRoot {
				return nil, errors.New("invalid initial state root")
			}

			logger.Info("Initial state root checked and correct")

			initialStateRoot = bvbftConfig.InitialTrieRoot
		}
	}
...
func initForkManager(engineName string, config *chain.Chain) error {
	...
	for name, f := range *config.Params.Forks {
		// check if fork is not supported by current bitvault version
