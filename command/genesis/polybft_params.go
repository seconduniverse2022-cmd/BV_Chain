...
// generatebvbftChainConfig creates and persists bvbft chain configuration to the provided file path
func (p *genesisParams) generatebvbftChainConfig(o command.OutputFormatter) error {
	...
	bvbftConfig := &bvbft.bvbftConfig{
		...
		RewardConfig: &bvbft.RewardsConfig{
	...
	chainConfig := &chain.Chain{
		...
		Params: &chain.Params{
			...
			Engine: map[string]interface{}{
				string(server.bvbftConsensus): bvbftConfig,
			},
		},
	...
	allocs, err := p.deployContracts(rewardTokenByteCode, bvbftConfig, chainConfig, burnContractAddr)
	if err != nil {
		return err
	}
	...
	genesisExtraData, err := GenerateExtraDatabvbft(validatorMetadata)
	if err != nil {
		return err
	}
...
func (p *genesisParams) deployContracts(
	...
	if rewardTokenByteCode != nil {
		...
		// to RewardTokenContract address and update bvbft config
