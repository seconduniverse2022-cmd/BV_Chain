...
func setFlags(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		...
		"the directory for the bitvault genesis data",
	)
	...
	// PoS
	{
		...
		cmd.Flags().StringArrayVar(
			...
			[]string{},
			"validators defined by user (bvbft format: <P2P multi address>:<ECDSA address>:<public BLS key>)",
		)
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	if params.isbvbftConsensus() {
		err = params.generatebvbftChainConfig(outputter)
	} else {
