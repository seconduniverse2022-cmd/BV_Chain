...
import (
	...
	"github.com/bitvault/consensus/ibft"
	"github.com/bitvault/consensus/ibft/fork"
	"github.com/bitvault/consensus/ibft/signer"
	"github.com/bitvault/contracts"
	"github.com/bitvault/contracts/staking"
	stakingHelper "github.com/bitvault/helper/staking"
	"github.com/bitvault/server"
	"github.com/bitvault/types"
	"github.com/bitvault/validators"
)
...
func (p *genesisParams) validateFlags() error {
	...
	if p.epochSize < 2 && (p.isIBFTConsensus() || p.isbvbftConsensus()) {
...
func (p *genesisParams) isbvbftConsensus() bool {
	return server.ConsensusType(p.consensusRaw) == server.bvbftConsensus
}
...
func (p *genesisParams) initRawParams() error {
	...
	if p.consensus == server.bvbftConsensus {
		return nil
	}
...
func (p *genesisParams) extractNativeTokenMetadata() error {
	tokenConfig, err := bvbft.ParseRawTokenConfig(p.nativeTokenConfigRaw)
	if err != nil {
		return err
	}
