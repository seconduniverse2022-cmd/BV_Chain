...
import (
	...
	"github.com/bitvault/helper/hex"
...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(
		&params.privateKey,
		bvbftsecrets.PrivateKeyFlag,
		"",
		bvbftsecrets.PrivateKeyFlagDesc,
	)
	...
	cmd.MarkFlagsMutuallyExclusive(bvbftsecrets.PrivateKeyFlag, bvbftsecrets.AccountConfigFlag)
	cmd.MarkFlagsMutuallyExclusive(bvbftsecrets.PrivateKeyFlag, bvbftsecrets.AccountDirFlag)
...
func runCommand(cmd *cobra.Command, _ []string) error {
	...
	if params.finalizeGenesisSet {
		...
		consensusConfig, err := bvbft.GetbvbftConfig(chainConfig)
		if err != nil {
		...
		genesisExtraData, err := genesis.GenerateExtraDatabvbft(validatorMetadata)
		if err != nil {
			return err
		}

		chainConfig.Genesis.ExtraData = genesisExtraData
		chainConfig.Params.Engine[bvbft.ConsensusName] = consensusConfig
...
func decodeGenesisAccounts(genesisSetRaw string) ([]*validator.GenesisValidator, error) {
	...
	decodbitvaultnesisSet, err := genesisSetABIFn.Outputs.Decode(genesisSetRawOut)
	if err != nil {
		return nil, fmt.Errorf("failed to decode genesis set from raw format: %w", err)
	}

	decodbitvaultnesisSetMap, ok := decodbitvaultnesisSet.(map[string]interface{})
	if !ok {
		return nil, errors.New("failed to convert genesis set to map")
	}

	decodbitvaultnesisSetSliceMap, ok := decodbitvaultnesisSetMap["0"].([]map[string]interface{})
	if !ok {
		return nil, errors.New("failed to convert genesis set to slice")
	}

	genesisAccounts := make([]*validator.GenesisValidator, len(decodbitvaultnesisSetSliceMap))
	for i, rawGenesisAccount := range decodbitvaultnesisSetSliceMap {
...
func initializeCheckpointManager(outputter command.OutputFormatter,
	...
	consensusConfig bvbft.bvbftConfig, chainID int64,
