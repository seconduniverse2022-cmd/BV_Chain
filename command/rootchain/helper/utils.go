...
import (
	...
	bvbftWallet "github.com/bitvault/consensus/bvbft/wallet"
...
func GetRootchainID() (string, error) {
	...
	for _, c := range containers {
		if c.Labels["bitvault-type"] == "rootchain" {
			return c.ID, nil
		}
	}
...
func GetECDSAKey(privateKey, accountDir, accountConfig string) (ethgo.Key, error) {
	...
	return bvbftWallet.GetEcdsaFromSecret(secretsManager)
}
...
func GetValidatorInfo(validatorAddr ethgo.Address, supernetManagerAddr, stakeManagerAddr types.Address,
	chainID int64, txRelayer txrelayer.TxRelayer) (*bvbft.ValidatorInfo, error) {
	...
	validatorInfo := &bvbft.ValidatorInfo{
